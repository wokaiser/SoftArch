...
public class PlaygroundFrame implements ActionListener {
	...
	private static final long serialVersionUID = -3520650782476117888L;
	private static final int ICON_WIDTH = 50;
	private static final int ICON_HEIGHT = 50;
	private static final ImageIcon CELL_INIT_ICON = new ImageIcon("resources/icons/cell_init.gif");
	private static final ImageIcon CELL_SHIP_ICON = new ImageIcon("resources/icons/cell_ship.gif");
	private static final ImageIcon CELL_HIT_ICON = new ImageIcon("resources/icons/cell_hit.gif");
	private static final ImageIcon CELL_MISS_ICON = new ImageIcon("resources/icons/cell_miss.gif");
	...
	private void initCells(char [][] playground, int row, int column) {		
		ownCells[row][column] = new JButton(getCellIcon(playground[row][column], CELL_SHIP_ICON));
		enemyCells[row][column] = new JButton(getCellIcon(playground[row][column], CELL_INIT_ICON));
		
		/* Action listener is only for NOT AI player */
		if (!controller.isEnemyAI()) {
    		enemyCells[row][column].addActionListener(this);	
		}
		ownCells[row][column].setPreferredSize(new Dimension(ICON_WIDTH, ICON_HEIGHT));
		enemyCells[row][column].setPreferredSize(new Dimension(ICON_WIDTH, ICON_HEIGHT));
	...
	private ImageIcon getCellIcon(char cell, ImageIcon defaultIcon) {
		...
		ImageIcon [] icons = {CELL_INIT_ICON, CELL_HIT_ICON, CELL_MISS_ICON};
	...
	public void sync() {
	    ...
	    for (int row = 0; row < this.controller.getRows(); row++) {
	    	for (int column = 0; column < this.controller.getColumns(); column++) {
				ownCells[row][column].setIcon(getCellIcon(playground[row][column], CELL_SHIP_ICON));
				enemyCells[row][column].setIcon(getCellIcon(playground[row][column], CELL_INIT_ICON));
	    	}
	    }
	}
